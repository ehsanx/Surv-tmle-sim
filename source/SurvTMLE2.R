...
surv.TMLE = function(dat, Yvar, Cvar, Avar, Lvar, L0var = NULL,
  ...
  if(nlevels(as.factor(dat[,Avar])) == 2){ # A is binary
    if(is.null(L0var)){ # No time-fixed covariates
      ...
      # --- Corrected Code ---
      predictor_names_A <- c(names(dat[, L0var, drop = FALSE]), # L0var is known to exist here
                             names(dat[, Lvar[[1]], drop = FALSE]))
      unique_predictors_A <- unique(predictor_names_A)
      print("--- DEBUG: Unique Predictors for A (Parametric) ---")
      print(unique_predictors_A) # Verify no duplicates
      outcome_name_A <- names(dat[,Avar, drop=FALSE])
      # Ensure outcome isn't predictor
      unique_predictors_A <- unique_predictors_A[unique_predictors_A != outcome_name_A]
      Aform = paste(outcome_name_A, "~", paste(unique_predictors_A, collapse = " + "))
      # --- End Corrected Code ---
    }
    if(Amod == "parametric"){
      ...
      # --- APPLY FIX HERE ---
      if (is.null(L0var)) { # No time-fixed covariates
        predictor_names_A <- names(dat[, Lvar[[1]], drop = FALSE])
        # No need for unique() here
      } else { # Some time-fixed covariates
        predictor_names_A <- c(names(dat[, L0var, drop = FALSE]),
                               names(dat[, Lvar[[1]], drop = FALSE]))
        unique_predictors_A <- unique(predictor_names_A) # Apply unique() HERE
        predictor_names_A <- unique_predictors_A # Use unique names
      }
      outcome_name_A <- names(dat[,Avar, drop=FALSE])
      # Ensure outcome isn't predictor
      predictor_names_A <- predictor_names_A[predictor_names_A != outcome_name_A]
      # Correct paste usage
      Aform = paste(outcome_name_A, "~", paste(predictor_names_A, collapse = " + "))
      # --- END FIX ---
      ...
      print("Fitting A model (SuperLearner)...") # ADD THIS LINE
      print(paste("Formula:", Aform))           # ADD THIS LINE
      #browser() # <--- ADD THIS LINE
      # Create the subset directly from dat
      X_data_subset <- dat[, predictor_names_A, drop = FALSE]
      print("Passing data subset with original names directly to SuperLearner:")
      print(paste("Columns:", paste(colnames(X_data_subset), collapse=", ")))
      print(paste("Dimensions:", nrow(X_data_subset), "x", ncol(X_data_subset)))
      # --- END ADD ---
      
      # Ensure A.SL.library is defined
      # Use SL.library from main call if A.SL.library not specified
      if (!exists("A.SL.library") || is.null(A.SL.library)) {
        if (!exists("SL.library") || is.null(SL.library)){
          stop("SL.library or A.SL.library must be provided when Amod = 'SL'")
        }
        A.SL.library <- SL.library
      }
      
      
      # Call SuperLearner with the data subset
      mod.A = SuperLearner(Y = dat[, Avar], X = X_data_subset, # <--- Pass data subset here
                           family = "binomial",
  ...
  if(is.null(L0var)){ # No time-fixed covariates
    if(Cmod == "parametric"){
      for(j in 1:K){
        ...
        # Replacement for the Cform definition around line 670:
        # --- Corrected Code for Cform (j > 1, SL, L0var present) ---
        predictor_names_C <- c(names(dat[,Avar, drop = FALSE]),
                               names(dat[, L0var, drop = FALSE]), # L0var is known to exist here
                               names(dat[, Lvarj, drop = FALSE]))
        unique_predictors_C <- unique(predictor_names_C)
        # Ensure outcome isn't predictor (using Cvar[j] name)
        outcome_name_C <- names(dat[,Cvar[j], drop=FALSE])
        unique_predictors_C <- unique_predictors_C[unique_predictors_C != outcome_name_C]
        Cform = paste(outcome_name_C, "~", paste(unique_predictors_C, collapse = " + "))
        # --- End Corrected Code ---
      ...
      for(j in 1:K){
        ...
        if(j == 1){ # First time point
          if(max(dat[,Cvar[j]]) == 0){ # No censoring
            ...
            names(X) = paste0("X", 1:ncol(X));
            suppressWarnings({
              print(paste("Fitting C model (SuperLearner), j =", j)) # ADD THIS LINE
              print(paste("Formula:", Cform))                        # ADD THIS LINE
              print("Colnames X for C:"); print(colnames(X))         # Optional: Add to see X columns
    ...
    if(Cmod == "parametric"){
      for(j in 1:K){
        ...
        #               paste(names(dat[, Lvarj, drop = FALSE]), collapse = " + "), sep = "");
        # Replacement for the Cform definition around line 577:
        # --- Corrected Code for Cform (Parametric, L0var present) ---
        predictor_names_C <- c(names(dat[,Avar, drop = FALSE]),
                               names(dat[, L0var, drop = FALSE]), # L0var is known to exist here
                               names(dat[, Lvarj, drop = FALSE]))
        unique_predictors_C <- unique(predictor_names_C)
    ...
    for(j in k:1){
      ...
      for(a in sort(unique(dat[,Avar]))){
        if(Ymod == "parametric"){
          ...
          #                 paste(names(dat[, Lvarj, drop = FALSE]), collapse = " + "), sep = "");
          # }
          # Combine all potential predictor names
          predictor_names <- c(names(dat[,Avar, drop = FALSE]),
                               names(dat[, Lvarj, drop = FALSE]))
          if (!is.null(L0var)) {
            predictor_names <- c(predictor_names, names(dat[, L0var, drop = FALSE]))
          }
          # Get the unique set of predictor names
          unique_predictors <- unique(predictor_names)
          # Ensure the outcome variable itself ('YSL') is not listed as a predictor
          unique_predictors <- unique_predictors[unique_predictors != "YSL"]
          
          # Construct the formula using unique predictor names
          Qform = paste("YSL ~", paste(unique_predictors, collapse = " + "))
          # --- End Corrected Code ---
          ...
          if(neff > 0){ # There were both events and survivals
            print(paste("Fitting Q model (SuperLearner), k =", k, "j =", j, "a =", a)) # ADD THIS LINE
            print(paste("Formula:", Qform))                                           # ADD THIS LINE
            print("Colnames X for Q:"); print(colnames(X))  
