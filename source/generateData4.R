generateData4 <- function(n, ntime = 4){
  expit <- plogis;
  ...
  A <- rbinom(n, size = 1, prob = plogis(-4.1 + 0.6*L + 0.6*L1 +
                                         0.4*L*L1 + # Interaction
                                         0.3*ifelse(L > 0, L^2, 0) + # Piecewise quadratic term 0.2*log(L_1^2 + 1) + ## Log transform
                                         0.5*I(L1 > quantile(L1, 0.75)))) # Threshold (top 25% of L1)
  ...
  for(j in 2:ntime){
    ...
    Lj = get(nameL);
    # assign(namepy1, ifelse(Yj_1.1 == 0, expit(-2 - 1 + 0.25*L + 0.25*Lj + 0.1*L*Lj - 0.2*L*1 + 0.2*Lj_1 + 0.1*Lj**2), 1));
    ## original version: assign(namepy1, ifelse(Yj_1 == 0, expit(-2 - 1 + 0.25*L + 0.25*Lj + 0.1*L*Lj - 0.2*L*1 + 0.2*Lj_1 + 0.1*Lj**2), 1));
    ## replaced Yj_1 with Yj_1.1, based on the definition by the paper: 
    ## Y^a_{t} = 1 if Y^a_{t-1} = 0
    ## for exposure A = 1
    assign(namepy1, ifelse(Yj_1.1 == 0, 
                           expit(-3 -1 + 0.25*L + 0.25*Lj +
                                 0.1*L*Lj - # Interaction
                                 0.2*L*1 +
                                 0.2*Lj_1 +
                                 0.1*Lj*Lj_1 + # Interaction
                                 0.1*Lj^2 +
                                 0.1*Lj^3 +  # Cubic term
                                 0.3*I(Lj > median(Lj)) +  # Threshold
                                 0.15*exp(0.2*Lj) +  # Exponential term
                                 0.1*sqrt(pmax(Lj, 0)) +  # Square root term
                                 0.05*A*exp(Lj) +  # Interaction with exposure
                                 0.2*pmax(Lj - 1, 0)),
                           1))  # Hinge function

    # assign(namepy0, ifelse(Yj_1.0 == 0, expit(-2 + 0 + 0.25*L + 0.25*Lj + 0.1*L*Lj - 0.2*L*0 + 0.2*Lj_1 + 0.1*Lj**2), 1));
    ## original version: assign(namepy0, ifelse(Yj_1 == 0, expit(-2 + 0 + 0.25*L + 0.25*Lj + 0.1*L*Lj - 0.2*L*0 + 0.2*Lj_1 + 0.1*Lj**2), 1));
    ## replaced Yj_1 with Yj_1.0, based on the definition by the paper: 
    ## Y^a_{t} = 1 if Y^a_{t-1} = 0
    assign(namepy0, ifelse(Yj_1.0 == 0, 
                           expit(-3 +0 + 0.25*L + 0.25*Lj +
                                 0.1*L*Lj - # Interaction
                                 0.2*L*0 +
                                 0.2*Lj_1 +
                                 0.1*Lj*Lj_1 + # Interaction
  ...
  var.names = c("id", "L", "A", paste0("Censor", 1:ntime), paste0("Y", 1:ntime),
                paste0("L", 1:ntime), paste0("Z", 1:4), paste0("Y", 1:ntime, ".1"), paste0("Y", 1:ntime, ".0")); 
