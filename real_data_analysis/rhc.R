...
# Define event indicator (1 = died; "Yes" in the source)
rhc_data$death01 <- ifelse(rhc_data$death == "Yes", 1, 0)

# --- Event-time construction (#70 corrected): in-hospital 30-day mortality ---
# Previously the event time was built from the DISCHARGE date
# (time = dschdte - sadmdte), which records a patient discharged alive who later dies
# within 30 days as dying on the discharge day. The corrected endpoint is in-hospital
# 30-day mortality: a death is in-hospital if a death date exists and falls on or before
# discharge (or no discharge is recorded); such deaths are timed at the DEATH date.
# Everyone else (discharged alive, including those who die after discharge) is censored at
# discharge, and anyone still alive in hospital at day 30 is administratively censored at 30.
# sadmdte / dschdte / dthdte are integer day-counts, so differences are in days.
n_time <- 30
t_disch <- as.numeric(rhc_data$dschdte - rhc_data$sadmdte)   # days admission -> discharge
t_death <- as.numeric(rhc_data$dthdte  - rhc_data$sadmdte)   # days admission -> death (NA if no death)

died_inhosp <- rhc_data$death01 == 1 & !is.na(t_death) &
               (is.na(t_disch) | t_death <= t_disch)
evt_time  <- ifelse(died_inhosp, t_death, NA_real_)
cens_time <- ifelse(died_inhosp, NA_real_, t_disch)   # left hospital alive at discharge

# Create discrete-time Y_t (event by t) and C_t (censored before t)
wide_data <- rhc_data

for (t in 1:n_time) {
  # Event at or before time t (in-hospital death timed at the death date)
  wide_data[[paste0("Y", t)]] <- ifelse(died_inhosp & !is.na(evt_time) & evt_time <= t, 1, 0)
  # Censored before time t (discharged alive, or still in hospital at the 30-day horizon)
  wide_data[[paste0("C", t)]] <- ifelse(!died_inhosp & !is.na(cens_time) & cens_time < t, 1, 0)
}
