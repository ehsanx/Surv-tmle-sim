...
for (col in L0var) {
  if (col %in% names(pbc_complete)) {
    ...
    # NOTE (sex fix): recode sex from its FACTOR levels before any numeric coercion.
    # A prior blanket as.numeric() here turned the factor levels f/m into 1/2, so
    # (1/2 == "m") was always FALSE and sex.male became all-zero -- silently dropping
    # sex from the nuisance models. We now recode sex directly from its factor levels.
    if (col == "sex") {
